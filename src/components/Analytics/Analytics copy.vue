...
async function fetchAndUpdateStatus (urls = baseUrlList.value) {
    if(!urls || urls.length ===0) return
    const promises = urls.map(fetchNotificationStats)
    const results = await Promise.all(promises)
    combineDataQuery(results)
}

const fetchNotificationStats = async (query, urls = baseUrlList.value) => {
    console.log('Fetching notification stats...');
    try {
        const res = await fetch(`${urls[0]}/notification_stats?query=${query}`);
        const data = await res.json();

        const typesData = errorTypesData.value.datasets[0].data;
        const columnData = errorColumnData.value.datasets[0].data;
        ...
        data.forEach(item => {
            ...
            if (irregularity) {
                for (const [key, index] of Object.entries(errorMapping)) {
...
function setupStatsInterval(mode) {
    ...
    fetchNotificationStats(mode === t('Day') ? 'now' : (mode === t('Week') ? 'weelk' : 'month'));
...
// Theo dõi thay đổi của lựa chọn để đổi hàm fetch tương ứng
watch(value, (newValue) => {
    setupStatsInterval(newValue)
})

// Clear interval khi component bị huỷ (tốt cho memory)
onUnmounted(() => {
    if (statsInterval) clearInterval(statsInterval)
})
