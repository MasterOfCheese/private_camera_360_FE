<template>
  <div
    ...
    <template v-if="camera?.name">
      ...
      <WebRTCStreamCard v-if="has_webrtc_stream" ref="webrtcCardRef" :key="camera.id + '-webrtc'" :camera="camera"
      ...
      <template v-if="show_error_overlay">
        <div class="absolute inset-0 flex flex-col items-center justify-center bg-red-900/80 p-3 text-center z-10">
          ...
          <p class="text-xs text-red-400 mt-1">ID: {{ camera?.id }} | {{ camera?.name }}</p>
          <span
            class="absolute right-0 top-0 p-1.5 text-xs text-gray-400 hover:text-white cursor-pointer pointer-events-auto pi pi-times z-10"
      ...
      <template v-if="!show_error_overlay">
        ...
        <div
          ...
          <div>
            <span class="text-xs text-gray-400 hover:text-white cursor-pointer pointer-events-auto pi pi-times"
        ...
        <div
          ...
          <div class="flex items-center space-x-3">
            ...
            <span v-if="has_low_quality_stream"
              class="pointer-events-auto cursor-pointer text-gray-400 hover:text-white inline-flex items-center gap-1"
              @click.stop="toggleQuality">
              <i class="pi pi-cog"
                 :title="isHighQuality ? 'High Quality' : 'Low Quality'">
              </i>
              <small class="text-xs">{{ isHighQuality ? 'HD' : 'SD' }}</small>
            </span>
            <span
...
const isHighQuality = ref(false) // Track current quality state

// --- Computed Properties ---
const has_webrtc_stream = computed(() => !!props.camera?.webrtc_ip || !!props.camera?.webrtc_ip_low)
const has_fallback_video = computed(() => !!props.camera?.fallback_video_url)
const has_low_quality_stream = computed(() => {
  return !!props.camera?.webrtc_ip_low && props.camera.webrtc_ip_low !== props.camera.webrtc_ip
})

const currentStreamUrl = computed(() => {
  // Use high quality URL if:
  // 1. High quality is selected OR
  // 2. Low quality URL isn't available
  if (isHighQuality.value || !props.camera?.webrtc_ip_low) {
    return `${props.camera?.webrtc_ip}/whep`
  } else {
    return `${props.camera.webrtc_ip_low}/whep`
  }
})

const toggleQuality = () => {
  isHighQuality.value = !isHighQuality.value
  // Khi thay đổi chất lượng, WebRTCStreamCard sẽ tự động reconnect
  // do đã thay đổi url prop của nó thông qua computed currentStreamUrl
}
