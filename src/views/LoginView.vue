      ...
      <div class="w-full max-w-4xl mx-auto bg-gray-800/60 backdrop-blur-xl rounded-xl shadow-2xl border border-white/20 overflow-hidden flex flex-col md:flex-row md:items-stretch"
          ...
          <div class="w-full md:w-2/3 lg:w-3/5 p-8 md:p-10 lg:p-12 md:border-l md:border-white/10">
              ...
              <div class="mt-6 pt-6 border-t border-white/10 space-y-3">
                  <!-- internal Login -->
                  <button
                      @click="handleOAuthLogin('internal')"
                      ...
                      :disabled="isLoading">
                      Login with VN Smart Factory System
                  </button>
              </div>
          </div>
      </div>
...
const OAUTH_CONFIGS = {
internal: {
  clientId: 'iic-ai',  // Hoặc lấy từ env
  authUrl: 'https://fiisw-cns.myfiinet.com/oauth-service/oauth/authorize',
  redirectUri: `${window.location.origin}${window.location.pathname.replace(/\/$/, '')}/#/auth`,
  scope: 'profile'
}
}
...
onMounted(async () => {
  ...
  // Kiểm tra nếu URL đang sai dạng (?code=...#/login?...)
  if (currentUrl.includes('?code=') && currentUrl.includes('#/login')) {
    try {
      const [basePart, hashPart] = currentUrl.split('#/')
      const paramsPart = basePart.split('?')[1]
      ...
      if (code && state) {
        //Tạo lại URL đúng dạng cho Vue Router
        const cleanBase = basePart.split('?')[0]
        const newUrl = `${cleanBase}#/auth?code=${code}&state=${state}`

        console.log('🔁 Redirecting to corrected OAuth callback URL:', newUrl)
        window.location.href = newUrl
        return
      }
    } catch (e) {
      console.warn('Error while fixing OAuth redirect URL:', e)
    }
  }

  // Nếu có returnUrl trong query
